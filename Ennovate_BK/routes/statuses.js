...
router.get("/home_timeline", async (req, res) => {
  ...
  for (var friend of following)
    friendIDs.push(new mongoose.Types.ObjectId(friend.friendID));
...
router.post("/v3/update", upload.single("novaImage"), async (req, res) => {
  ...
  if (req.body.in_reply_to_status_id) {
    ...
    if (!inreply)
      return res
        .status(404)
        .send({ msg: "The Nova with the given ID was not found" });
  }
...
router.get("/user_timeline/:screen_name", async (req, res) => {
  ...
  if (!user)
    return res
      .status(404)
      .send({ msg: "The user with the given screen name was not found." });
...
router.post("/renova", async (req, res) => {
  ...
  await User.update(
    { _id: decoded._id },
    { $push: { novas_IDs: nova1._id }, $inc: { novas_count: 1 } },
    { new: true }
  );

  await Nova.update(
    { _id: nova._id },
    { $inc: { renova_count: 1 }, $push: { renovaed_by_IDs: user._id } }
  );
...
router.post("/destroy", async (req, res) => {
  ...
  if (nova.renovaed)
    return res
      .status(401)
      .send({ msg: "bad request this is a renova not a nova" });
  ...
  if (user._id.equals(user2._id)) {
    ...
    if (nova.in_reply_to_status_id) {
      await Nova.update(
        { _id: nova.in_reply_to_status_id },
        { $inc: { reply_count: -1 }, $pull: { replied_novas_IDs: novaid } }
      );
    }
    // normal nova
    ...
    await User.update(
      { _id: user._id },
      { $pull: { novas_IDs: novaid }, $inc: { novas_count: -1 } }
    );
    ...
    return res
      .status(404)
      .send({ msg: "No authentication to delete this nova" });
});
...
router.post("/unrenova", async (req, res) => {
  ...
  if (!nova)
    return res
      .status(404)
      .send({ msg: "The reNova with the given ID was not found." });
  // check 3ala el renovaed boolean
  if (!nova.renovaed)
    return res.status(401).send({ msg: "bad request this is not a renova" });

  let user2 = await User.findOne({ _id: nova.user });
  // console.log(user.screen_name == user2.screen_name);
  if (user.screen_name === user2.screen_name) {
    // console.log("LOL");
    const lengthReply = nova.reply_count;
    if (lengthReply !== 0) {
      var inreplyarray = [];
    ...
    await Nova.update(
      { _id: nova.in_reply_to_status_id },
      { $pull: { renovaed_by_IDs: novaid }, $inc: { renova_count: -1 } }
    );
    ...
    res.status(404).send({ msg: "Incorrect User" });
  }
});
...
router.post("/update", async (req, res) => {
  ...
  if (inreplytostatusid) {
    ...
    if (inreplynova) {
      ...
      return res
        .status(404)
        .send({ msg: "The Nova to reply to was not found" });
    }
  } else {
    inreplyuserid = null;
  ...
  if (req.body.user_mentions_count) {
    if (req.body.user_mentions_count !== 0) {
      if (req.body.user_mentions_screen_names) {
        ...
        for (let i = 0; i < req.body.user_mentions_count; i++) {
          let mentionuser = await User.findOne({
            screen_name: mentionsArray[i]
          });
          if (mentionuser) {
            mentionsId.push(mentionuser);
          } else {
            return res.status(404).send({
              msg: "The user " + mentionsArray[i] + " mentioned was not found"
            });
          }
        }
      } else {
        return res
          .status(404)
          .send({ msg: "bad request the mentions screen names was not sent " });
      }
    } else {
      mentionsId = null;
    }
  ...
  if (inreplyuserid) {
    await Nova.update(
      { _id: inreplytostatusid },
      { $push: { replied_novas_IDs: nova._id }, $inc: { reply_count: 1 } },
      { new: true }
    );
  }
  await User.update(
